...
{
    private const string CedulaPattern = @"^\d{11}$";
    ...
    public static Identify Create(string value)
    {
        ...
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(normalized, CedulaPattern))
        ...
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(normalized, RncPattern))
